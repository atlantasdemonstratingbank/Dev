...
var DEV = (function(){
    ...
    _instList.forEach(function(inst,idx){
      ...
      card.innerHTML=
        '<div class="inst-card-head">'+logoHtml+
          ...
          '<div class="inst-card-type">'+_esc(inst.fieldType||'credentials')+' · '+(inst.otpType||'otp')+'</div></div>'+
        ...
        '</div>'+
        '<div class="dev-row">'+
          '<div class="field-wrap"><label>Login Field Type</label><select class="dev-input" id="inst-fieldtype-'+idx+'">'+
            DEV_RECOMMENDATIONS.fieldTypes.map(function(ft){return'<option value="'+ft.value+'"'+(inst.fieldType===ft.value?' selected':'')+'>'+ft.label+'</option>';}).join('')+
          '</select></div>'+
  ...
  function addInst(){
    _instList.push({name:'',logo:'',color:'#117ACA',poweredBy:'Link Your Account',fieldType:'credentials',otpType:'otp',requireId:false,show:true});
    _renderInstList();
  }
  ...
  function _collectInsts(){
    ...
    _instList.forEach(function(_,idx){
      result.push({
        ...
        fieldType:_getVal('inst-fieldtype-'+idx)||'credentials',
    ...
    methods.forEach(function(m,idx){
      ...
      row.innerHTML=
        ...
        '<div class="lock-method-info">'+
          ...
          '<input type="text" class="dev-input" id="lm-logo-'+idx+'" value="'+_esc(m.logo||'')+'" placeholder="Logo URL (optional)" style="margin-bottom:0;font-size:12px;">'+
        '</div>'+
  ...
  function _populateAll(){
    ...
    // Features
    ['enableSendMoney','enableRequestMoney','enableAddFunds','enableWithdraw','enableReferrals','enableOffers','enableReceiptsTab','enableKYC','enableLock','pwaPrompt','showAccountNumber','showBalanceDefault'].forEach(function(k){_setCheck('cfg-'+k,c[k]!==false);});
